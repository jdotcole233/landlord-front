...
const RoomCard = ({
  ...
  const navigateToRoom = (id, tenants, room_name, room_number, apartment_name, rooms) => {
     history('/room', {
      state: {
        id,
        tenants,
  ...
  return (
    <div className="border shadow-xl lg:mb-0 mb-5">
      ...
      <div className="px-3">
        <h1 className="text-blue-700 font-bold text-lg">{room_name}</h1>
        ...
        <h1 className="text-blue-500">
          ...
          {tenants.length
            ? `${tenants[0]?.currency} ${tenants[0]?.monthly_rent}`
            : 0.0}
        </h1>
        <h1 className="text-red-500">
          <span className="text-gray-500 mr-1">Expires</span>
          {tenants.length ? moment("2023-03-12").fromNow() : "Empty"}
        </h1>
        <h1 className="text-blue-500">
          <span className="text-gray-500 mr-1">Total Tenants:</span>
          {<CountUp end={tenants.length} />}
        </h1>
      </div>
      <div className="flex px-2 py-2">
        ...
        <button
          onClick={() => navigateToRoom(id, tenants,  room_name, room_number, apartment_name, rooms)}
          className="border hover:shadow-lg hover:bg-slate-900 bg-slate-700 cursor-pointer px-2 rounded flex justify-center items-center py-1 text-white mr-1"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
